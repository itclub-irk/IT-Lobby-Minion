...
from src.states import FSM_DynamicButtons
...
async def static_buttons(message: Message, state: FSMContext):
    ...
    await message.answer(text='Редактирование динамических кнопок', reply_markup=keyboards.working_with_buttons_kb)
    r = await state.get_state()
    logger.debug(r)


@router.message(F.text == 'Получить список всех кнопок', FSM_DynamicButtons.working_with_buttons)
async def get_all_buttons(message: Message):
    await message.answer(
            'Список динамических кнопок',
            reply_markup=keyboards.static_or_dynamic_buttons_kb(settings.DYNAMIC_BUTTONS_CONFIG_NAME)
    )


@router.message(FSM_DynamicButtons.working_with_buttons, F.text == 'Удалить лишние кнопки')
async def delete_buttons(message: Message, state: FSMContext):
    ...
    r = await message.answer(
            text='<b>!!!Режим удаления кнопок!!!</b>\n\n'
                 'Через 30 секунд он автоматически выключится\n\n'
                 'Нажми на кнопку и она исчезнет',
            reply_markup=keyboards.welcome_message_kb_without_urls(settings.DYNAMIC_BUTTONS_CONFIG_NAME),
    )
...
async def delete_inline_button(qq: CallbackQuery):
    repository.remove_button(qq.data, button_config_name=settings.DYNAMIC_BUTTONS_CONFIG_NAME)
    await qq.message.edit_text(
            text=qq.message.html_text,
            reply_markup=keyboards.welcome_message_kb_without_urls(
                    buttons_config_name=settings.DYNAMIC_BUTTONS_CONFIG_NAME)
    )


@router.message(F.text == 'Добавить новую кнопку', FSM_DynamicButtons.working_with_buttons)
...
async def get_button_link(message: Message, state: FSMContext):
    ...
    repository.add_button(name=button_name, url=button_link, button_config_name=settings.DYNAMIC_BUTTONS_CONFIG_NAME)
